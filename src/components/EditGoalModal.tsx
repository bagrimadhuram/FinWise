...
import { X, Target, Calendar, IndianRupee, Loader2, Trash2 } from 'lucide-react';
...
export default function EditGoalModal({ goal, onClose }: { goal: Goal, onClose: () => void }) {
  const { user } = useAuth();
  const [name, setName] = useState(goal.name);
  const [targetAmount, setTargetAmount] = useState(goal.targetAmount.toString());
  const [currentSaved, setCurrentSaved] = useState(goal.currentSaved.toString());
      ...
      >
        <div className="p-8 border-b border-white/5 flex justify-between items-center bg-[#141414] text-white">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-2xl font-display font-medium tracking-tight">Edit Goal</h2>
          </div>
        ...
        <div className="p-8 space-y-6 bg-[#0a0a0a]">
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <div className="relative">
                  <span className="absolute left-6 top-1/2 -translate-y-1/2 text-xl font-bold text-teal-400">₹</span>
                  <input 
                    type="number" 
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <select 
                  className="w-full px-6 py-4 bg-white/5 border border-white/10 rounded-2xl font-bold text-white outline-none"
                  value={status}
          ...
          >
            {isSubmitting ? <Loader2 className="w-6 h-6 animate-spin" /> : 'Update Financial Goal'}
          </button>
        </div>
      </motion.div>
    </div>
